...
func GetTable() gin.HandlerFunc {
  return func(c *gin.Context) {
    ...
    table,
    err:= models.GetTableById(ctx, tableId)
    if err != nil {
      c.JSON(http.StatusInternalServerError, gin.H {
...
func CreateTable() gin.HandlerFunc {
  return func(c *gin.Context) {
    ...
    createdTable,
    err:= models.CreateTableDb(ctx, table)
    if err != nil {
      c.JSON(http.StatusInternalServerError, gin.H {
...
func UpdateTable() gin.HandlerFunc {
  return func(c *gin.Context) {
    ...
    if table.Guests != nil {
      updateObj = append(updateObj, fmt.Sprintf("guests=$%d", len(values)+1))
